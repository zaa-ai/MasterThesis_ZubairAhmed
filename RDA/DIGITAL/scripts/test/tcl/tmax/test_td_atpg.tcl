...
##        
check_var COVERAGE_TD_ATPG            "100"
check_var LIMIT_TD_ATPG               "10"
check_var CUSTOM_TD_ADD_FAULTS_SCRIPT ""

##################################################################
#    TD_ATPG: Create Patterns for Transition-Delay Test          #
##################################################################
set PATTERN_NAME "trans"
...
if {$CUSTOM_TD_ADD_FAULTS_SCRIPT != ""} {
   source -echo $CUSTOM_TD_ADD_FAULTS_SCRIPT
} else {
   add_faults -all
}
...
set_atpg -coverage $COVERAGE_TD_ATPG

## ELMOS: added variable (optional list) for ATPG limit
set_atpg -abort_limit [lindex $LIMIT_TD_ATPG 0] ; #use first element from list
set ADDITIONAL_LIMITS [lreplace $LIMIT_TD_ATPG 0 0] ; #remove first element from list
