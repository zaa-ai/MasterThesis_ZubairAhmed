...
##        
check_var COVERAGE_SA_ATPG            "100"
check_var LIMIT_SA_ATPG               "10"
check_var CUSTOM_SA_ADD_FAULTS_SCRIPT ""

##################################################################
#    SA_ATPG: Create Patterns for Stuck-At Fault Test            #
##################################################################
set PATTERN_NAME "stuck"
...
if {$CUSTOM_SA_ADD_FAULTS_SCRIPT != ""} {
   source -echo $CUSTOM_SA_ADD_FAULTS_SCRIPT
} else {
   add_faults -all
}
...
set_atpg -coverage $COVERAGE_SA_ATPG

## ELMOS: added variable (optional list) for ATPG limit
set_atpg -abort_limit [lindex $LIMIT_SA_ATPG 0] ; #use first element from list
set ADDITIONAL_LIMITS [lreplace $LIMIT_SA_ATPG 0 0] ; #remove first element from list
...
echo "TetraMAX II ATPG can change the clock matrix." >> ${REPORTS_DIR}/${DESIGN_NAME}_stuck_clocks.${tmax_file_ext}.rpt
echo "The post-ATPG clock matrix is:" >> ${REPORTS_DIR}/${DESIGN_NAME}_stuck_clocks.${tmax_file_ext}.rpt
