...
##        
check_var COVERAGE_CT_SLACK_ATPG            "100"
check_var LIMIT_CT_SLACK_ATPG               "10"
check_var CUSTOM_CT_SLACK_ADD_FAULTS_SCRIPT ""

########################################################################
#  CT_SLACK_ATPG: Create Patterns for Cell-Aware Transition-Delay Test #
########################################################################
## ELMOS: This script is copied from "test_td_atpg.tcl" and
##        adapted for Cell Aware
set PATTERN_NAME "cell_trans_slack"
...
if {$CUSTOM_CT_SLACK_ADD_FAULTS_SCRIPT != ""} {
    source -echo $CUSTOM_CT_SLACK_ADD_FAULTS_SCRIPT
} else {
    ...
    ## ELMOS: extract value for max_tmgn in [ns]
    ##
...
set_atpg -coverage $COVERAGE_CT_SLACK_ATPG

## ELMOS: added variable (optional list) for ATPG limit
set_atpg -abort_limit [lindex $LIMIT_CT_SLACK_ATPG 0] ; #use first element from list
set ADDITIONAL_LIMITS [lreplace $LIMIT_CT_SLACK_ATPG 0 0] ; #remove first element from list
