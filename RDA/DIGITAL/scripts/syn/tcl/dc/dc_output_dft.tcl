...
read_test_protocol -section test_setup -test_mode ScanCompression_mode ctl/${DESIGN_NAME}.scancompress_init.ctl 
...
read_test_protocol -section test_setup -test_mode Internal_scan        ctl/${DESIGN_NAME}.scan_init.ctl  
set_dft_drc_configuration -internal_pins disable
...
current_test_mode Internal_scan
read_test_protocol -section test_setup -test_mode Internal_scan        ctl/${DESIGN_NAME}.iddq_init.ctl -overwrite
set_dft_drc_configuration -internal_pins disable
...
#ELMOS: use this section only if OCC is enabled:
if { ${USE_OCC} == TRUE } {
    # For on-chip clocking, enable the PLL bypass mode during post-DFT DRC
    set_dft_drc_configuration -pll_bypass enable
    
    dft_drc
    dft_drc -verbose > ${REPORTS_DIR}/${DCRM_DFT_DRC_FINAL_PLL_BYPASS_REPORT}
