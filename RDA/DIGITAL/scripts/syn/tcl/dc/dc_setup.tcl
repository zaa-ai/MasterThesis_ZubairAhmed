...
if {$synopsys_program_name == "dc_shell" || $synopsys_program_name == "dcnxt_shell"}  {
  ...
  # ELMOS: added additional variables
  
  # do not allow latches in netlist (there is no equivalent for blackboxes)
  #set_attribute hdl_error_on_blackbox true
  set_app_var hdlin_check_no_latch   true
  
  # shorten module names, to be able to read in the design to Custom Compiler
  set_app_var hdlin_shorten_long_module_name true
  set_app_var hdlin_module_name_limit 128
  
  # enable timing analysis between all specified clocks
  set_app_var timing_enable_multiple_clocks_per_reg true
  
  # enable non-standard timing checks
  ...
  # avoid tri statements in netlists
...
# ELMOS: added standard paths and variable for excluding paths in autoread
set RTL_SOURCE_FILES  "../.. ${TECH}/hdl"      ;# Enter the list of source RTL files if reading from RTL
set AUTOREAD_EXCLUDE_LIST "be_run tb fv"
...
#set OPTIMIZATION_FLOW                 ""    ;# Specify one flow out of hplp | hc | rtm_exp | tim -> is set in common_setup.tcl 
set REDUCED_EFFORT_OPTIMIZATION_FLOW  false ;# Use reduced effort for lower runtime in hplp and tim flows (false | true)
#################################################################################
# Search Path Setup
#
...
#################################################################################
  ...
  # ELMOS: added user_tcl and def directories
  set_app_var search_path ". ${ADDITIONAL_SEARCH_PATH} $search_path user_tcl def sdc"
#################################################################################
# Library Setup
#
...
#################################################################################
  ...
  set ndm_design_library ${DCRM_NDM_LIBRARY_NAME}
