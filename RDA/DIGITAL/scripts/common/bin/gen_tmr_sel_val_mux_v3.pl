      ...
      if ($in_entity){
        ...
        if ($line=~ m/^\s*wire\s*(\[\S+\])\s*(\w+)/){  # grep input signals width and name
          ...
          if ($line=~ m/TMR_SEL_VAL/){               # grep input signals width and name
            ...
            $inputs->{COMB} = "SEPA";
            $inputs->{COMBN} = 1;
            if ($line=~ m/TMR_SEL_VAL\s+.*\s+COMB/){
              $inputs->{COMB} = "COMB";
              if ($line=~ m/TMR_SEL_VAL\s+.*\s+COMB\s*\((\d+)\)/){
                $inputs->{COMBN} = $1;
              } else {
                $inputs->{COMBN} = 1;
              }
...
# -------------
  ...
  print "`include \"$module_name\_inc.v\"\n\n";

  print "module $module_name\_mux #(\n";        # create new module
  print "    parameter DOMAIN_3V0 = 0\n";
  print "  )(\n";
  print "    input  [`$module_name_uc\_MUX_SEL_MSB:0] tmr_sel,\n";
  print "    input  [`$module_name_uc\_MUX_VAL_MSB:0] tmr_val,\n";
  print "\n";
  print "    $if_module_name.dp io_dp,\n";
  print "    $if_module_name.ap io_ap\n";
  print "  );\n\n";
...
# -------------
  ...
  foreach my $listitem(@inputlist){
    if ($listitem->{TMR_SEL_VAL}){
      ...
      if ($listitem->{COMB} eq "COMB") {
        $width_msb = $listitem->{COMBN} - 1;
      }
  ...
  print INC "`define $module_name_uc\_MUX_VAL_MSB $tmr_val_msb\n";
  print INC "`define $module_name_uc\_MUX_SEL_MSB $tmr_sel_msb\n";
  print INC "\n";
  ...
  foreach my $listitem(@inputlist){
    if ($listitem->{TMR_SEL_VAL}){
      ...
      if ($listitem->{COMB} eq "COMB") {
        if (($width_msb + 1) % $listitem->{COMBN} != 0) {
          die("ERROR: Vector $listitem->{NAME} width " . ($width_msb + 1) . " is not a multiple of COMB number $listitem->{COMBN} !\n");
        }
        my $num = int(($width_msb + 1) / $listitem->{COMBN});
        print "    .i_a ({" . $num . "{tmr_val[`B_TMR_MUX_VAL_\U$listitem->{NAME}]}}),\n";
        $width_msb = $listitem->{COMBN} - 1;
      } else {
      ...
      if ($listitem->{DIR} =~ m/A2D/) {
        print "    .i_b (io_ap.$listitem->{NAME}),\n";
        print "    .i_sa(tmr_sel[`B_TMR_MUX_SEL_\U$listitem->{NAME}]),\n";
        print "    .o_y (io_dp.$listitem->{NAME})\n";
      } else {
        print "    .i_b (io_dp.$listitem->{NAME}),\n";
        print "    .i_sa(tmr_sel[`B_TMR_MUX_SEL_\U$listitem->{NAME}]),\n";
        print "    .o_y (io_ap.$listitem->{NAME})\n";
      }
      print "  );\n";
      print "\n";
