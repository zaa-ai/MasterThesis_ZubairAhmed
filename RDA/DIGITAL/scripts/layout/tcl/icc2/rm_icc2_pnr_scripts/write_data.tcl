...
# ELMOS: avoid longer names to be able to read in verilog to Custom Compiler
define_name_rules LVS -case_insensitive -max_length 128
change_names -rules LVS -hierarchy

change_names -rules verilog -hierarchy

# ELMOS: patch time stamp to an instance name of the netlist
set id_fill [index_collection [get_cells -filter "is_physical_only == true" *] 0]
set time_stamp [exec date +%Y%m%d_%H%M]
set ID "IDENT_${time_stamp}_${DESIGN_NAME}_$env(USER)"
set_attribute $id_fill name $ID
...
set write_verilog_lvs_cmd "write_verilog -compress gzip -force_no_reference [get_lib_cells $NO_OUTPUT_REFERENCES] -exclude {scalar_wire_declarations leaf_module_declarations empty_modules} -hierarchy all ${OUTPUTS_DIR}/${WRITE_DATA_BLOCK_NAME}.lvs.v"
