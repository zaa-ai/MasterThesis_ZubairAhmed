...
##############################################################################################################################
## For ODL MCMM flow, it is required to specify a valid $PNA_TLUPLUS_MAX_FILE or $PNA_TLUPLUS_MIN_FILE for PNA or it will fail.
if {$PNA_TLUPLUS_MAX_FILE != "" || $PNA_TLUPLUS_MIN_FILE != ""} {
  set set_tlu_plus_files_cmd "set_tlu_plus_files -tech2itf_map $MAP_FILE"
  if {$PNA_TLUPLUS_MAX_FILE != ""} {
  	lappend set_tlu_plus_files_cmd -max_tluplus $PNA_TLUPLUS_MAX_FILE
  }
  if {$PNA_TLUPLUS_MIN_FILE != ""} {
  	lappend set_tlu_plus_files_cmd -min_tluplus $PNA_TLUPLUS_MIN_FILE
  }
eval $set_tlu_plus_files_cmd
} else {
  echo "RM-Error: You need to specify a valid \$PNA_TLUPLUS_MAX_FILE or \$PNA_TLUPLUS_MIN_FILE for PNA or it will fail."
}
...
if { [check_error -verbose] != 0} { echo "RM-Error, flagging ..." }
  ...
  echo "RM-Info: Sourcing [which $ICC_DP_MCMM_SCENARIOS_FILE]"
  source -echo $ICC_DP_MCMM_SCENARIOS_FILE
  }
