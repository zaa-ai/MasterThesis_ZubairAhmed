...
if {$ICC_ECO_FLOW == "FREEZE_SILICON"} {
  ...
  if {[file exists [which $ICC_ECO_FILE]]} {
    ...
    # ELMOS:
    if {[file extension $ICC_ECO_FILE] == ".v"} {
      # Verilog flow
      eco_netlist -compare_pg -preserve_routing -freeze_silicon -by_verilog_file $ICC_ECO_FILE -write_changes eco_changes.tcl
    } elseif {[file extension $ICC_ECO_FILE] == ".tcl"} {
      # TCL flow
      eco_netlist -freeze_silicon -by_tcl_file $ICC_ECO_FILE
    } else {
      echo "Error: ECO can't be performed as $ICC_ECO_FILE has no valid file extension [.v|.tcl]..."
    }
  ...
  if { [check_error -verbose] != 0} { echo "RM-Error, flagging ..." }
    ...
    # Places new cell instances by swapping out spare cells.
    ...
    # ELMOS
    # The insert_diodes_during_routing might be active in the used milkyway,
    # due to FREEZE_SILICON is wanted, new diodes unwanted
    set_route_zrt_detail_options -antenna true -insert_diodes_during_routing false
    
    # ELMOS: recommended to try -> example from TSMC BCD Gen 2
    # set_route_zrt_common_options -freeze_layer_by_layer_name {{METAL4 true} {METAL5 true}}
    
    # ELMOS: recommended to try
    # route_zrt_eco -reroute modified_nets_first_then_others -reuse_existing_global_route true
