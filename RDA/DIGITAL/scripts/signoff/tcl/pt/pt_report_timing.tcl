...
eval report_global_timing ${ADDITIONAL_OPTIONS} > $REPORTS_DIR/${DESIGN_NAME}_${SCENARIO}_report_global_timing.report
report_clock -skew -attribute > $REPORTS_DIR/${DESIGN_NAME}_${SCENARIO}_report_clock.report 
report_analysis_coverage > $REPORTS_DIR/${DESIGN_NAME}_${SCENARIO}_report_analysis_coverage.report
eval report_timing ${ADDITIONAL_OPTIONS} -crosstalk_delta -slack_lesser_than 0.0 -delay min_max -nosplit -input -net  > $REPORTS_DIR/${DESIGN_NAME}_${SCENARIO}_report_timing.report
eval report_constraints ${ADDITIONAL_OPTIONS} -all_violators -verbose > $REPORTS_DIR/${DESIGN_NAME}_${SCENARIO}_report_constraints.report
report_design > $REPORTS_DIR/${DESIGN_NAME}_${SCENARIO}_report_design.report
report_net > $REPORTS_DIR/${DESIGN_NAME}_${SCENARIO}_report_net.report
# Clock Network Double Switching Report
report_si_double_switching -nosplit -rise -fall > $REPORTS_DIR/${DESIGN_NAME}_${SCENARIO}_report_si_double_switching.report

# Noise Settings
set_noise_parameters -enable_propagation -analysis_mode report_at_endpoint
check_noise > $REPORTS_DIR/${DESIGN_NAME}_${SCENARIO}_check_noise.report
update_noise
# Noise Reporting
report_noise -nosplit -all_violators -above -low > $REPORTS_DIR/${DESIGN_NAME}_${SCENARIO}_report_noise_all_viol_abv_low.report
report_noise -nosplit -nworst 10 -above -low > $REPORTS_DIR/${DESIGN_NAME}_${SCENARIO}_report_noise_alow.report

report_noise -nosplit -all_violators -below -high > $REPORTS_DIR/${DESIGN_NAME}_${SCENARIO}_report_noise_all_viol_below_high.report
report_noise -nosplit -nworst 10 -below -high > $REPORTS_DIR/${DESIGN_NAME}_${SCENARIO}_report_noise_below_high.report


# ELMOS: added for transition patterns
report_global_slack -significant_digits 4 -nosplit > $REPORTS_DIR/${DESIGN_NAME}_${SCENARIO}_report_global_slack.report
